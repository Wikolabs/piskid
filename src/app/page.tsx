...
function Aloalo({ size = 200, color = C.earth, opacity = 0.08 }: { size?: number; color?: string; opacity?: number }) {
  return (
    <svg width={size} height={size * 2.4} viewBox="0 0 80 192" fill="none" style={{ opacity }}>
      {/* Crowning sun/circle */}
      <circle cx="40" cy="14" r="10" stroke={color} strokeWidth="1.6" fill="none" />
      {/* Zebu horns crescent */}
      <path d="M22 30 Q 30 22 40 24 Q 50 22 58 30" stroke={color} strokeWidth="2.2" fill="none" strokeLinecap="round" />
      {/* Diamond carving */}
      <path d="M40 38 L 52 50 L 40 62 L 28 50 Z" stroke={color} strokeWidth="1.4" fill="none" />
      {/* Inner cross */}
      <path d="M40 44 L 40 56 M34 50 L 46 50" stroke={color} strokeWidth="1.2" />
